...
### 🔒 Security & Search Improvements

### Changed
- **Dependency Security Update** - Upgraded `mammoth` from `^1.8.0` to `^1.12.1` to address the directory traversal advisory and keep the extension on a supported version
- **Search Result Counter** - Added browser-like find count metadata so the viewer shows current match position alongside total matches, e.g. `1/32`
- **Improved Search Highlighting** - Current match is visually emphasized while remaining matches stay highlighted for quick navigation

### Fixed
- **Search Navigation UX** - Search panel now keeps match count in sync after typing, navigating, and clearing the query
